...
import 'package:wafarly/features/home/presentation/admin_views/widgets/add_store_texts_fields.dart';
...
class AddStoreBottomSheet extends StatelessWidget {
  const AddStoreBottomSheet({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<StoresCubit,StoresState>(
      listener: (context, state) {
        ...
        }else if(state is AddStoreFailureState){
          showErrorSnackBar(context: context, message: state.error);
        }
      },
      builder: (context, state) {
      return ModalProgressHUD(
        inAsyncCall: state is AddStoreLoadingState,
        color: Colors.white,
        opacity: 0.5,
        child: Container(
                ...
                child: Column(
                  children: [
                    const AppBarBottomSheet(
                      title: AppStrings.addStore,
                    ),
                    Gap(AppConstants.size20h),
                    const AddStoreTextsFields(),
                  ],
                ),
              ),
              SliverFillRemaining(
                    ...
                    CustomElevatedButton(
                      onPressed: () {
                        if (StoresCubit.get(context)
                            ...
                            .currentState!
                            .validate()) {
                          StoresCubit.get(context).addStore();
                        }
                      },
